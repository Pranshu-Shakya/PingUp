		...
		<div className="min-h-screen bg-slate-50">
			<div className="max-w-6xl mx-auto p-6">
                ...
                <div className="mb-8 flex flex-wrap gap-6">
                    {
                        dataArray.map((item, index) => (
                            <div className="flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-white shadow rounded-md" key={index}>
                ...
                <div className="inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm">
                    {
                        dataArray.map((tab) => (
                            <button key={tab.label} className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? "bg-white font-medium text-black" : "text-gray-500 hover:text-black"}`} onClick={() => setCurrentTab(tab.label)}>
                                <tab.icon className="w-4 h-4" />
                                <span className="ml-1">{tab.label}</span>
                                {
                                    tab.count !== undefined && (
                                        <span className="ml-2 text-xs bg-gray-700 px-2 py-0.5 rounded-full">{tab.count}</span>
                                    )
                                }
                            </button>
                        ))
                                    ...
                                    <div className="flex max-sm:flex-col gap-2 mt-4">
                                        ...
                                        }
                                        {
                                            currentTab === 'Following' && (
                                                <button className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 cursor-pointer transition">Unfollow</button>
                                            )
                                        }
                                        {
                                            currentTab === 'Pending' && (
                                                <button className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 cursor-pointer transition">Accept</button>
                                            )
                                        }
                                        {
