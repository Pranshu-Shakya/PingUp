...
// discover users using username, email, locatio and name
...
export const unfollowUser = async (req, res) => {
	try {
		...
		return res
			.status()
			.json({ success: true, message: "You are no longer following this user." });
	} catch (error) {
		console.log(error);
		return res.status(500).json({ success: false, message: error.message });
	}
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if(connectionRequests.length >= 20) {
            return res.status(429).json({ success: false, message: "You have exceeded the connection request limit. Please try again later." });
        }
